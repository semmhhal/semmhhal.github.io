
//use this file for any code you want to try out
let Student:{studentID:number,quizAnswers:number[]}
let student1={studentID:101,quizAnswers:[1,1,2,4]}
let student2={studentID:101,quizAnswers:[2,1,2,2]}
let student3={studentID:101,quizAnswers:[3,1,3,4]}

console.log("expect 3: " computestudentScore(student1,[3,1,2,4]))
//get the student score
function computestudentScore(stu:Student,arr:number[]):number{
    const answers=stu.quizAnswers;
    //loop through the parallel arrays and count the matches and return that

    let numCorrect=0;

    for(let i=0;i<answers.length;i++){
        if(answers[i]===quizKey[i]){
            numCorrect=numCorrect+1
        }
    }
    return numCorrect
}
student2;
student3;