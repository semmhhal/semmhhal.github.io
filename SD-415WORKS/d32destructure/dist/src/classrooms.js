...
export function collectLabeledRoomCaps() {
    let Output = classrooms.map((currentElement) => ({ roomNumber: currentElement.roomNumber, capacity: currentElement.capacity }));
    return Output;
}
export function countStudentsInClassroom(classrooms, num) {
    let count = 0;
    for (let elements of classrooms) {
...
export function findClassroomsWithCapacity(classrooms, capacity) {
    return classrooms.filter((classroom) => classroom.capacity >= capacity);
}
export function findStudentsOlderThan(classrooms, age) {
    let result = [];
